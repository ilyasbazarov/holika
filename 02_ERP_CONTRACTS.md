...
**⚠ Часть строк — forward-fill, а не наблюдённый курс (`ADR-049 §3`).** `cf-fx` делает forward-fill автоматически при 401 от Bakai (`PROJECT_REFERENCE_v6` стр.186/757); ручной forward-fill `INSERT` — штатная процедура `RUNBOOK_v8 §13.4`. Источник (официальный курс НБКР) в выходные не публикуется — `RUNBOOK_v8` журнал инцидентов: 2026-05-13 («FX lag > 3 дня (выходные)» → forward-fill 4 строки), 2026-05-18 (`М-19`, lag 7 дней → 7 строк), 2026-05-21 (`М-24`), 2026-06-03 (`М-28`, 7 строк). Покрытие без единой дыры на 454 днях означает, что не-биржевые дни заполнены копией предыдущего курса. **Доля ЗАМЕРЕНА (`ADR-057 §1`; `/reference/fx_forward_fill_2026-07-26.md`, прогон 2026-07-26T14:46:56Z).** Дней, в которые значение не менялось: 63,3 % (апрель-2026), **96,8 %** (май-2026), 86,7 % (июнь-2026), 73,1 % (июль-2026, 1–26). **Май-2026 несёт ОДНО значение `87.4205` на все 31 день** — вошло 2026-05-01, внутримесячных изменений ноль. Копий в рабочие дни — 13–20 в месяц, то есть основная часть не объясняется выходными. Миграция источника 2026-06-03 режим качественно не изменила ⇒ это **действующее** состояние таблицы, а не только историческая особенность. Метод даёт верхнюю границу forward-fill, не доказательство (`ADR-053 §6`).

**Причина малоподвижности НЕ установлена — `Q-69`:** две неразличённые гипотезы (потеря изменений загрузчиком `cf-fx`/его источником против действительно малоподвижного официального курса). Различитель назначен `ADR-058 §2/§3` (GCS-архив ответа источника + логи `cf-fx`), задача — `briefs/FX-RATE-FIDELITY.md`.

**Следствие для потребителей до закрытия `Q-69`:** `rate_kgs_per_usd` на дату `D` нельзя трактовать как курс дня `D`. Для мая-2026 варианты «по курсу на дату операции» и «по последнему курсу на конец мая» дают одно и то же число (`ADR-057 §4`). Выбор политики исходящей конвертации — DEFER (`ADR-057 §3`).

**Следствие для потребителей:** `rate_kgs_per_usd` на дату `D` не гарантированно есть курс дня `D` — это курс последнего дня, когда источник ответил, не позднее `D`. Материально для выбора политики исходящей конвертации (`ADR-039 §2`, `ADR-049 §4`, `ADR-053 §5`).
