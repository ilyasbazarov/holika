...
**DQ Gate — 7 чеков (PR-31 + PR-19 + `DQ-GATE-METRIC-REDESIGN`, ADR-153/ADR-173), CF `cf-dq`:**

| Чек | Порог | Механизм + scope | Что проверяет |
|---|---|---|---|
| not_empty | `staging_count > 0` | cf-dq / DQ Gate, автомат, блокирующий | Staging не пустой |
| drift_check | будни ≥0.10, выходные ≥0.03 | cf-dq / DQ Gate, автомат, блокирующий; **T-1 rev / MA7(T-8…T-2)**, не T-0 (правило 25) | Отклонение выручки от скользящей средней — только класс «документы есть, выручка ниже нормы» (`ADR-153 §1`, второй исход ниже) |
| drift_zero_docs | нет (диагностика, `passed` всегда `true`) | cf-dq / DQ Gate, автомат, **notify-only, не блокирует promote** (`ADR-153`) | Второй исход `drift_check` — сутки без документов вовсе (`target_rev = 0`); внутри есть тот же различитель `ADR-152` (пустое окно T-8…T-2 при непустой истории CORE — вероятная остановка промоута — блокирует через `drift_check`, не через этот чек) |
...
**Второй исход `drift_check` (`DQ-GATE-METRIC-REDESIGN`, ADR-153 кандидат 1, деплой `DQ-CFDQ-DEPLOY`, ADR-173, ревизия `cf-dq-00009-coy`).** Метрика больше не отождествляет «документов не было вовсе» (`target_rev = 0`) с «данные испорчены» — до фикса это давало `ratio = 0` и ложно блокировало promote (`8,9 %` суток истории). Теперь `target_rev = 0` уходит в отдельный notify-only чек `drift_zero_docs` (строка выше), а блокирующий `drift_check` сужен до класса «документы есть, выручка ниже нормы». Внутри ветки `target_rev = 0` сохранён различитель `DQ-GATE-FAIL-OPEN-FIX` (`ADR-152`): если окно T-8…T-2 тоже пусто при непустой истории CORE (вероятная остановка промоута), возврат остаётся блокирующим через `drift_check`, а не тихо уходит в `drift_zero_docs` — тихое снятие защиты `ADR-152` для нулевых дней было найдено и закрыто ревью (`ADR-173 §1`). Пороги `0,10`/`0,03` не менялись. Вторая лог-метрика для доставки исхода `drift_zero_docs` в телеграм-канал `notify` — отдельная задача класса B, не входит в этот деплой (`ADR-153 §Последствия`).
...
### §политика исходящей FX-конвертации (`ADR-062`, владелец, 2026-07-26)

**Класс.** Исходящая конвертация — готовая итоговая сумма в KGS переводится в USD для инвестора через `core.dim_fx_rates`. К входящей конвертации (`rate.value` документа МойСклада, `ADR-010`/`ADR-028`) не относится и с ней не смешивается.

**Политика — по типу метрики (вариант `(c′)`, `ADR-050 §2`).** Единой формы на все марты нет сознательно.

**Критерий отнесения — наличие дневного зерна в строке витрины.** Критерий проверяем по SQL и не требует отдельного решения при появлении новой витрины (`ADR-062 §2`).

| Витрина | Зерно строки | Форма курса | Основание |
|---|---|---|---|
| `marts.expenses` | `moment` / месяц / неделя — дневное зерно есть | **по дате операции**, с деградацией на последний курс | исторический поток; воспроизводимость величины закрытого периода (`00_CHARTER §миссия`) |
| `marts.in_transit` | состояние на текущий момент | последний курс (`latest_fx`) | оценка товара в пути сейчас; исторический курс здесь содержательно неверен |
| `marts.inventory_health` | последний снапшот (`date_snapshot = MAX(...)`) | последний курс (`latest_fx`) | то же |
| `marts.gmroi` | `(period_days, product_id)`, окна 30/90 дней — **дневного зерна нет** | последний курс + пометка качества | «по дате» неисполнимо без пересборки агрегата и переопределения метрики; в одной строке смешаны поток и состояние |
| `marts.abc_xyz` | `product_id`, окно 90 дней — **дневного зерна нет** | последний курс + пометка качества | то же |
| `marts.gmroi_by_folder` | наследует `MAX(rate_kgs_per_usd)` из `marts.gmroi` | наследует | своего запроса к `dim_fx_rates` нет — не пункт политики (`ADR-050 §3`) |
| `marts.supplier_price_history` | `order_date` | по дате с деградацией — **уже реализовано** | эталон формы для `expenses` (`ADR-050 §4`) |

**Форма «по дате с деградацией» — эталон в репо, не изобретение** (`ADR-062 §5`): `LEFT JOIN core.dim_fx_rates fx ON fx.date = <дата строки>` плюс `COALESCE` на последний курс при отсутствии строки, как в `marts.supplier_price_history`.

**Документированная граница** (`ADR-062 §6`): `core.dim_fx_rates.min_date = 2025-04-29` (`02 §core.dim_fx_rates`). Для дат ранее величина деградирует на последний курс ⇒ воспроизводимость для самого раннего периода не достигается. **Замер исполнен (задача `FX-POLICY-EXPENSES`, 2026-07-28, `/reference/fx_policy_expenses_2026-07-28.md`):** в `core.fact_payments` таких платежей `1027` (`MIN(moment) = 2024-04-15`, более чем за год до `min_date`); проверка по всем трём источникам (`core.fact_payments`, `core.fact_loss`, `core.fact_commissionreportin`) подтвердила, что строки без курса на свою дату лежат **строго раньше** `2025-04-29` — начиная с этой даты все три источника находят курс на дату операции без единого пропуска.

**Пометка качества курса** обязательна для витрин без дневного зерна (`gmroi`, `abc_xyz` и наследующий `gmroi_by_folder`): USD-величина посчитана по курсу на момент пересборки марта, а не по курсу периода. Колонка `rate_kgs_per_usd` в этих витринах уже присутствует; форма явной пометки назначается задачей реализации (`ADR-062 §8`).

**Приведённый ниже канонический SQL `in_transit` настоящей политике соответствует и правке не подлежит** (`ADR-062 §10`).
...
Распределение X/Y/Z и A-класс SKU — волатильные цифры, дом `07_STATE` §Контрольные цифры (факт **2026-08-01**: X=28, Y=164, Z=338 при 530 строках `marts.abc_xyz`; A=112, B=151, C=267).
...
`sq_marts_expenses` (Config ID `6a22a243-0000-20fd-a458-883d24f4cad4`) — живой SQL на момент выгрузки 2026-07-07, читает `core.fact_payments` (payment-based, только 2 из 4 источников ADR-006: `paymentout`+`cashout`). Документированный оракул-разрыв ~33% (ADR-005 §4). **Статус (ADR-012, supersedes ADR-009 §1): канон продакшена.** Прод-таблица `marts.expenses` строится ЭТИМ SQL; НЕ ретайрится — ПРАВИТСЯ точечно там, где расхождение подтверждено (Epic-1/E1-T1).

Полный SQL — см. `/reference/sql/sq_marts_expenses.sql` (дословный снапшот, не воспроизводится второй раз в спеке).

**Кросс-ссылка (методология правок, ADR-012):** `§marts.expenses` ниже (ADR-006, двухисточниковая агрегация `paymentout+cashout+loss` + `entity/commissionreportin`) — целевая методология для точечных правок ЭТОГО SQL (в первую очередь — добавить недостающие `loss`+`commissionreportin`), НЕ спецификация отдельно строящегося марта.

*(PR-236, ADR-009 §1 partially superseded by ADR-012)*
...
**Выходная таблица:** `marts.expenses` — та же таблица, что сейчас строит легаси `sq_marts_expenses` (`§marts — SQL` выше). Методология этой секции не создаёт отдельную таблицу — она описывает целевую логику для точечных правок существующего SQL (ADR-012).
...
Счётчики записей (волатильные) → `07_STATE` §контрольные цифры (см. STATE_PATCH этой сессии). Полный список `EXCLUDE_EXPENSE_IDS` — вне scope, **GAP Q-10** (канон в коде, закрывается вместе с Q-3). Формализовано как **ADR-011** (`06_DECISIONS_LOG.md`, статус: accepted) — канон: только `DELETE`-after-`MERGE`, `if/continue` на выгрузке запрещён (конформность живого кода — гейт Q-3, не верифицирована).
