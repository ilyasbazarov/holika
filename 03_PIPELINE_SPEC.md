...
**Документированная граница** (`ADR-062 §6`): `core.dim_fx_rates.min_date = 2025-04-29` (`02 §core.dim_fx_rates`). Для дат ранее величина деградирует на последний курс ⇒ воспроизводимость для самого раннего периода не достигается. Фактическое наличие таких строк в `core.fact_payments` замеряется задачей `FX-POLICY-EXPENSES`; результат вносится сюда.
