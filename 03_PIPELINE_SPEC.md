...
Счётчики записей (волатильные) → `07_STATE` §контрольные цифры (см. STATE_PATCH этой сессии). Полный список `EXCLUDE_EXPENSE_IDS` — вне scope, **GAP Q-10** (канон в коде, закрывается вместе с Q-3). Кандидат в ретро-ADR (P5) — Ghost Records fix как формальное решение задним числом.
