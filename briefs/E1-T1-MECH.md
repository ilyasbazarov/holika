...
**Шаг 0б — остаточные перепроверки. Порядок жёсткий (`ADR-047 §1`), отклонение в первой отменяет остальные.**

Гейт заливки истории уже снят фактом: `core.fact_loss` = 128, `core.fact_commissionreportin` = 191
(`07_STATE §Блокеры`, `B-10`). Ниже — то, что стало проверяемым только теперь.

1. **Регрессия по маю.** Три суммы за май обязаны остаться прежними: `core.fact_payments`
   8 600 919,01 · `core.fact_loss` при `applicable = TRUE` 1 193 254,77 · `core.fact_commissionreportin`
   438 729,42 · итог 10 232 903,20. Любое отклонение = **СТОП всей задачи** (`ADR-047 §2`): это
   расхождение с референс-оракулом, закрывается правилом / override / ADR по `ADR-002`, не подгонкой.
   Дальше не идти.
2. `applicable = FALSE` в истории (`ADR-034`) — есть ли такие строки вообще.
3. Попадают ли четыре системных `expense_item_id` (`ADR-037`) в историю `core.fact_loss`.
4. Варианты написания названий статей (`ADR-035`). Запрос обязан **назвать выборку явно** (какие
   именно таблицы) и **напечатать сами значения**, а не только их число: провенанс исходного замера
   «29 различных названий» не воспроизводим (`ADR-047 §6`, `ADR-044 §2`).
5. `COUNTIF(project_id IS NOT NULL)`, `COUNTIF(sales_channel_id IS NOT NULL)`,
   `COUNTIF(agent_name IS NOT NULL)` по `core.fact_loss` (`ADR-036` / `Q-55`).

**Что делать с результатом (`ADR-047 §3`).** Любая из проверок 2–5, давшая результат против премиссы
своего ADR, переоткрывает **этот** ADR **отдельным новым ADR ДО** правки SQL, не после. Оптом не
решать (прецедент `ADR-020 §5`). Текст исходного ADR не редактируется — `06` append-only. Для
`ADR-036` переоткрытие узкое: меняется правило заполнения ветки списаний, форма выхода `ADR-032`
(17 колонок, имена, порядок) не затрагивается (`ADR-047 §5`).

**Ограничение, без которого выводы 2–4 будут переоценены (`ADR-047 §4`).** Референс-оракул посеян
**только за май-2026** (`ADR-005`, `/reference/pnl_2026-05.md`; иные периоды → P5, `Q-1`). Для месяцев
кроме мая изменение сумм от фильтров сверять не с чем. Результат проверок 2–4 на не-майской истории —
это факт о наличии/отсутствии строк, **не приёмка корректности сумм**. Приёмка `E1-T1-MECH` остаётся
майской (`ADR-033`). Не докладывать «сошлось» там, где эталона не существует.
