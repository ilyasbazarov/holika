-- test_merge.sql — SALES-MERGE-DRYRUN, вопрос (а)
-- Воспроизводит _build_merge_sql (reference/code/cf-facts/bq_ops.py:176-303) дословно,
-- cogs_source_table = core.fact_sales_profit_byvariant_backup (hourly, window_days=7 — как в живом дефолте).
-- ЕДИНСТВЕННОЕ ОТЛИЧИЕ от живого запроса: добавлена строка
--   T.transaction_date = S.transaction_date
-- в блок WHEN MATCHED THEN UPDATE SET — это и есть предмет проверки (допустима ли UPDATE
-- партиционирующей колонки внутри MERGE для этой таблицы). ON, список колонок INSERT (C1/ADR-030),
-- источник S — сохранены без изменений против живой формы.
