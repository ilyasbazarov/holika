...
**Слияние ветки в `master` код-репо:** гейт слияния по `05_CONVENTIONS.md §Процедура деплоя…` —
успешная проверка развёрнутого архива (условие 1, выполнено); подтверждение владельца на
конкретное действие слияния запрошено отдельно (не выполнено этой записью).
