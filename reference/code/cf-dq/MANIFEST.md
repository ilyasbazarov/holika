...
`deploy/cf-dq-2026-08-18-freshness-wire-v2` (коммит `16d93ec`) запушена в `origin`. Неудачная
ветка первой попытки `deploy/cf-dq-2026-08-18-freshness-wire` (`2a3228d`) остаётся провенансом.

**Слияние в `master` (решение архитектора, подтверждено владельцем явным «да» в чате,
2026-08-18).** Деплой принят, база ветки (`c9b967f`) уже была предком `master` без дрейфа —
слияние чисто техническая гигиена, в отличие от `cf-facts` (там `master` намеренно держится
как точка отката до ступени 3, `2026-08-23`). Слито обычным `git merge` (без конфликтов, ровно
один файл `cf-dq/main.py`), запушено: `master` `9a08b5a` → `9d9ecba`
(`Merge remote-tracking branch 'origin/deploy/cf-dq-2026-08-18-freshness-wire-v2'`). `master`
код-репо теперь равен обслуживающей ревизии `cf-dq-00011-tij`.
