...
def check_drift(bq):
    ...
    row = run_row(bq, f"""
        WITH max_d AS (
            SELECT MAX(DATE(PARSE_TIMESTAMP('%Y-%m-%d %H:%M:%E3S', transaction_date_raw),
                           'Asia/Bishkek')) AS d
            FROM `{STAGING}`
        )
        SELECT
            CAST(max_d.d AS STRING) AS max_date,
            EXTRACT(DAYOFWEEK FROM max_d.d) AS day_of_week,
            COALESCE(SUM(s.revenue_kgs), 0) AS today_rev
        FROM `{STAGING}` s CROSS JOIN max_d
        WHERE DATE(PARSE_TIMESTAMP('%Y-%m-%d %H:%M:%E3S', s.transaction_date_raw),
                   'Asia/Bishkek') = max_d.d
        GROUP BY max_d.d
    """)
    if not row or row.get("max_date") is None:
        return False, "max_date=NULL (staging пустой)"
    today_rev  = float(row.get("today_rev", 0) or 0)
    ...
    """) or 0.0
    if ma7 == 0:
        return True, f"today_rev={today_rev:.0f}, ma7=0 (нет истории → пропуск)"
    ratio = today_rev / float(ma7)
    return (ratio >= threshold,
            f"today_rev={today_rev:.0f}, ma7={float(ma7):.0f}, ratio={ratio:.2f}, "
            f"threshold={threshold} ({day_label}), max_date={max_date}")
