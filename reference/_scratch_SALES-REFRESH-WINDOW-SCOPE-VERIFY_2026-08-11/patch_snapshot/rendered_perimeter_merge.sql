...
USING (
  ...
  FROM `msklad-bi-prod.stg_msklad.fact_sales_perimeter_staging` s
  LEFT JOIN `msklad-bi-prod.core.fact_sales_profit_byvariant_backup` b
    ON  s.product_id = b.product_id
    AND DATE_TRUNC(DATE(PARSE_TIMESTAMP('%Y-%m-%d %H:%M:%E3S', s.transaction_date_raw), 'Asia/Bishkek'), WEEK(SATURDAY)) = b._week_start
...
WHEN NOT MATCHED BY SOURCE
  ...
  AND COALESCE(T.sales_channel_name, '') IN ('Розница', 'Комиссия')
THEN DELETE
